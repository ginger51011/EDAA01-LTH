...
public class TestSimpleHashMap {
	...
	public final void testIntegerMinValue() {
		...
		assertEquals("wrong size():", 2, m.size());
		assertEquals("key not found in map: Integer.MIN_VALUE", new Integer(17), m.get(Integer.MIN_VALUE));
	}
	...
	public final void testManyPutAndRemove() {
		...
		SimpleHashMap shm16 = (SimpleHashMap) m16;
		System.out.println(shm16.show());
		for (int i : randNbrs) {			
			assertEquals("key not found in map:" + i, new Integer(i), m16.remove(i));
		}
		shm16 = (SimpleHashMap) m16;
		System.out.println(shm16.show());
		assertEquals("wrong size():", 0, m16.size());
	}
